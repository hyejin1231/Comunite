            ...
            public void onSessionOpened() {
                ...
                UserManagement.getInstance().me(new MeV2ResponseCallback() {
                    ...
                    public void onSuccess(MeV2Response result) {
                        ...
                        databaseReference.child("UserAccount").orderByChild("et_email").equalTo(result.getKakaoAccount().getEmail()).addListenerForSingleValueEvent(new ValueEventListener() {
                            @Override
                            public void onDataChange(@NonNull  DataSnapshot snapshot) {
                                ...
                                String email = snapshot.child(key).child("et_email").getValue().toString();

                                if (email.equals(result.getKakaoAccount().getEmail())) {
                                    ...
                                    account.setIdToken(unique);
                                    account.setNickName(result.getKakaoAccount().getProfile().getNickname());
                                    account.setEt_email(result.getKakaoAccount().getEmail());
                                    account.setProfile(result.getKakaoAccount().getProfile().getProfileImageUrl());
                                    account.setKakao("o");

                                    databaseReference.child("UserAccount").child(unique).setValue(account);
                                }
                            }
...
//                            Intent intent = new Intent(LoginActivity.this, MainActivity.class);
                            ...
                            databaseReference.child("UserAccount").orderByChild("et_email").equalTo(checkid).addListenerForSingleValueEvent(new ValueEventListener() {
                                @Override
                                public void onDataChange(@NonNull  DataSnapshot snapshot) {
                                    for (DataSnapshot child : snapshot.getChildren()) {
                                        key2 = child.getKey();
                                    ...
                                    if (key2 == null) {
                                        ...
                                        String idToken = Cuser.getUid();

                                        UserAccount userAccount = new UserAccount();
                                        userAccount.setIdToken(idToken);
                                        userAccount.setEt_email(email);
                                        userAccount.setNickName(nickName);
                                        userAccount.setProfile(profile);
                                        userAccount.setKakao("x");

                                        databaseReference.child("UserAccount").child(Cuser.getUid()).setValue(userAccount);

                                        Toast.makeText(LoginActivity.this, "등록완료", Toast.LENGTH_SHORT).show();
                                    }
